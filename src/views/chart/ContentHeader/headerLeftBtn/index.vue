<template>
  <n-space class="header-left-btn" :wrap="false" :size="25">
    <n-button v-show="false" size="small" quaternary @click="goHomeHandle()">
      <template #icon>
        <n-icon :depth="3">
          <home-icon></home-icon>
        </n-icon>
...
const btnList = reactive<ItemType<ChartLayoutStoreEnum>[]>([
  {
    ...
    select: getCharts,
    title: t('dashboard.chart_component'),
    icon: renderIcon(BarChartIcon)
  },
  {
    key: ChartLayoutStoreEnum.LAYERS,
    select: getLayers,
    title: t('dashboard.layer_control'),
    icon: renderIcon(LayersIcon)
  },
  {
    key: ChartLayoutStoreEnum.DETAILS,
    select: getDetails,
    title: t('dashboard.detail_setting'),
